...
def checkFilter(predicted_labels,files_path,filter_dict,text_file):
       ...
       # print(cluster_files)
        ...
        if(len(set(cluster_files)&set(trump_rnc))!=0):
           # print("writing to a text file")
            ...
            text_file.write(f'Number of images in trump cluster-> {cluster_num}-> {len(set(cluster_files)&set(trump_rnc))} out of {len(set(cluster_files))} \n')
        ...
        if(len(set(cluster_files)&set(flyn))!=0):
            ...
            text_file.write(f'Number of images in flynn cluster-> {cluster_num}-> {len(set(cluster_files)&set(flyn))} out of {len(set(cluster_files))} \n')
            filter_dict['flyn']+=1
        ...
        if(len(set(cluster_files)&set(khewsis))!=0):
            ...
            text_file.write(f'Number of images in khewsis cluster->{cluster_num}-> {len(set(cluster_files)&set(khewsis))} out of {len(set(cluster_files))} \n')
            filter_dict['khewsis']+=1
            
        
    print(f"Total clusters for trump rnc is {filter_dict['trumprnc']}\n")
    print(f"Total clusters for flynn is {filter_dict['flyn']}\n")
    print(f"Total clusters for khewsis rnc is {filter_dict['khewsis']}\n")
    ...
    text_file.close()
    if(filter_dict['trumprnc']>10):
        return False
    
    if(filter_dict['flyn']>10):
        return False
    
    if(filter_dict['khewsis']>10):
        return False
...
if __name__ == '__main__':
	...
	files_path.sort()
	print(files_path[0:10])

	for file in list(os.listdir(predicted_labels_path)):
	#for file in ['ideology_model_vgg_16.npy-umap_dbscan_labels.npy']:
		if file.endswith('.npy'):
			print(file)
			filter_dict={"flyn":0,"trumprnc":0,"khewsis":0}
			predicted_labels=np.load(f'{predicted_labels_path}{file}',allow_pickle=True)
			unique_labels=set(predicted_labels)
